...
<div class="detail-hero">
    ...
    <img src="<?= upload_url("users/{$user['id']}/brushes/{$brush['hero_image']}") ?>" alt="<?= e($brush['name']) ?>"
         ...
         data-full="<?= upload_url("users/{$user['id']}/brushes/{$brush['hero_image']}") ?>">
    <?php else: ?>
    ...
    <div>
        ...
        <div class="detail-section">
            ...
            <div class="image-gallery">
                ...
                <div class="image-gallery-item"><img src="<?= upload_url("users/{$user['id']}/brushes/" . str_replace('.', '_thumb.', $image['filename'])) ?>" alt="Additional image" loading="lazy"
                     class="clickable-image"
                     data-gallery="brush-<?= $brush['id'] ?>"
                     data-full="<?= upload_url("users/{$user['id']}/brushes/{$image['filename']}") ?>"></div>
                <?php endforeach; ?>
            </div>
        </div>
        <?php endif; ?>
...
<div class="mt-4"><a href="<?= url('/share/' . e($token) . '/brushes') ?>" class="btn btn-outline">&larr; Back to Brushes</a></div>
